...
class Bank:
    ...
    def search_customer(self, last_name: str, first_name: str) -> Customer | None:
        for customer in self.__customers:
            if customer.lasname == last_name and customer.firstname == first_name:
                return customer
    ...
    def delete_customer(self, last_name: str, first_name: str) -> None:
        ...
        i = 0
        while i < len(self.__customers):
            if self.__customers[i].lasname == last_name and self.__customers[i].firstname == first_name:
                self.__customers[i] = self.__customers[-1]
                self.__customers.pop()
                break
            i += 1
    ...
    def find_mini_balance(self) -> Customer | None:
        min_balance = 0
        min_customer = None
        for customer in self.__customers:
...
def main():
    ...
    min_customer = bank.find_mini_balance()
    print(min_customer)
    print()
